...
export function toolFeatures(env = process.env): ToolFeatures {
  return {
...
type ToolRegistrar = { registerTool: (...args: any[]) => unknown };

export function registerHelpTool(server: ToolRegistrar, backend: Backend, features: ToolFeatures): void {
  ...
  }, async ({ topic }: { topic: keyof typeof topics }) => ({
    content: [{ type: "text" as const, text: topics[topic].join("\n") }],
  }));
}
