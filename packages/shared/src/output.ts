...
export function outputResponse(text: string, result: { stdout: string; stderr: string; [key: string]: unknown }) {
...
export function commandOutputResponse<T extends { stdout: string; stderr: string; [key: string]: unknown }>(
  result: T,
  format: (result: T) => string,
  options: { limit?: number; outputDir?: string } = {},
) {
  ...
  const notes: string[] = [];
  let fullOutput: { stdout: string; stderr: string } | undefined;
