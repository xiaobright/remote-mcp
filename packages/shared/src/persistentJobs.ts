...
export function readPersistentJobStore(storePath: string): PersistentJobStore {
  try {
    ...
    const raw = parsed as { version?: unknown; jobs?: unknown };
    ...
    return {
      version: 1,
      jobs: raw.jobs as Record<string, PersistentJobRecord>,
    };
  } catch (error) {
...
function writePersistentJobStoreUnlocked(storePath: string, store: PersistentJobStore): void {
...
export function writePersistentJobStore(storePath: string, store: PersistentJobStore): void {
  withPersistentJobStoreLock(storePath, () => writePersistentJobStoreUnlocked(storePath, store));
}

export function upsertPersistentJob(
  storePath: string,
...
export function getPersistentJob(storePath: string, jobId: string): PersistentJobRecord {
  const store = readPersistentJobStore(storePath);
...
export function listPersistentJobs(storePath: string): PersistentJobRecord[] {
  const store = readPersistentJobStore(storePath);
  return Object.values(store.jobs).sort((a, b) => a.startedAt.localeCompare(b.startedAt));
}

export function touchPersistentJob(
  storePath: string,
  jobId: string,
  patch: Partial<PersistentJobRecord>,
): PersistentJobRecord {
  return withPersistentJobStoreLock(storePath, () => {
    const store = readPersistentJobStore(storePath);
...
export function deletePersistentJob(storePath: string, jobId: string): void {
...
export function resolveDefaultPersistentJobStorePath(moduleDir: string): string {
...
export function newPersistentJobRecord(
  options: Pick<PersistentJobRecord, "jobId" | "backend" | "maxRuntimeMs" | "workdir" | "target" | "requestedTarget" | "configuredDistro" | "sshOptions">,
): PersistentJobRecord {
...
export function requireJobCommandSuccess(action: string, result: { exitCode: number; stderr: string; timedOut?: boolean }): void {
...
export function inferPersistentJobState(statusContent: string): PersistentJobState {
 ...
 * tabs or newlines).
 */
export function buildPersistentJobInspectScript(options: PersistentJobInspectOptions): string {
...
export function decodeJobPage(base64: string, offset: number, nextOffset: number, total: number, mayGrow: boolean) {
...
export function parsePersistentJobInspect(raw: string): PersistentJobInspectResult {
  ...
  for (const key of ["STATUS", "STDOUT_LEN", "STDERR_LEN", "STDOUT_OFFSET", "STDOUT_NEXT", "STDOUT_B64", "STDERR_OFFSET", "STDERR_NEXT", "STDERR_B64"]) {
    if (!map.has(key)) throw new Error(`Invalid job inspection response: missing ${key}. Remote state is unknown; do not restart the job blindly.`);
  }
