    ...
    if (line.startsWith("*** Update File:")) {
      ...
      while (index < lines.length && !lines[index].startsWith("*** ")) {
        ...
        if (/^@@ -\d+(,\d+)? \+\d+(,\d+)? @@/.test(currentLine.trim())) {
          throw new Error(
            `Unified diff hunk header "${currentLine}" is not supported. Use a bare @@ as the hunk separator; line counts are not needed.`,
          );
        }
        if (/^(--- |\+\+\+ |diff --git )/.test(currentLine)) {
          throw new Error(
            `Unified diff file header "${currentLine}" is not supported. This tool uses the Codex patch format: `
