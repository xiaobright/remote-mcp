...
function blockOps(a: string[], b: string[], oldBase: number, newBase: number): DiffOp[] {
  const ops: DiffOp[] = [];
...
function fileHeaders(path: string): [string, string] {
  if (path.startsWith("/")) {
    return [`--- ${path}`, `+++ ${path}`];
  }
...
export function unifiedDiff(
  ...
  const lines: string[] = fileHeaders(path);
  let truncated = false;
  let clusterStart = 0;
