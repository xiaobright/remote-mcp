...
function isConsistentCrlf(text) {
    if (!text.includes("\r\n")) {
        return false;
    }
...
function allMatchLines(work, oldString) {
    const lines = [];
...
function findWhitespaceInsensitiveMatch(fileLines, oldLines) {
...
function findClosestRegion(fileLines, oldLines) {
...
function mismatchError(path, oldString, work, options) {
    ...
    if (strippedEnd.length > 0 && work.includes(strippedEnd)) {
        return new Error(`Edit did not match ${path}: old_string is not found verbatim, but it matches after removing its trailing newline(s). Retry with old_string without the trailing "\\n".${fixHint}`);
    }
    if (strippedStart.length > 0 && strippedStart !== strippedEnd && work.includes(strippedStart)) {
        return new Error(`Edit did not match ${path}: old_string is not found verbatim, but it matches after removing its leading newline(s). Retry with old_string without the leading "\\n".${fixHint}`);
    }
    ...
    if (closest) {
        parts.push(`Closest region in file (lines ${closest.startLine}-${closest.endLine}, similarity ${(closest.score * 100).toFixed(0)}%):`, formatNumberedLines(closest.fileLines, closest.startLine), `old_string (escaped): ${escapeText(oldString)}`);
    }
    else {
    ...
    if (options.replaceAll) {
        ...
        if (second >= 0) {
            const locations = allMatchLines(work, oldString)
                .map((line) => `line ${line}`)
                .join(", ");
            throw new Error(`Edit matched multiple locations in ${path} (${locations}). Pass replace_all=true to replace every occurrence, or add more surrounding lines to old_string so it matches exactly one.`);
        }
