...
export async function readTextFile(target: RemoteTarget, path: string, maxBytes = 5 * 1024 * 1024): Promise<string> {
  const script = `set -eu
...
export async function writeTextFile(target: RemoteTarget, options: {
  path: string;
  content: string;
...
export async function statPath(target: RemoteTarget, path: string): Promise<RemoteFileInfo> {
  const script = `set -eu
path=${shellQuote(path)}
...
export async function listDir(target: RemoteTarget, path: string): Promise<RemoteListEntry[]> {
  const script = `set -eu
path=${shellQuote(path)}
if [ ! -d "$path" ]; then
...
export async function searchText(target: RemoteTarget, options: {
  path: string;
  pattern: string;
  fixed?: boolean;
  maxResults?: number;
