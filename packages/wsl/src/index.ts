...
function formatDistro(distro: string | null): string {
...
function formatTaskStarted(task: { taskId: string; configuredDistro: string | null; pid: number | null }): string {
...
async function handleSessionAction(params: { action: string; distro?: string }) {
  switch (params.action) {
    ...
    case "start": {
      ...
      return {
        content: [{ type: "text" as const, text: `WSL session started for ${formatDistro(state.configuredDistro)}.` }],
        structuredContent: state,
      };
    }
    case "stop": {
      ...
      return {
        content: [{ type: "text" as const, text: `WSL session stopped. Configured distro remains ${formatDistro(state.configuredDistro)}.` }],
        structuredContent: state,
      };
    }
    case "set_distro": {
...
function optionalRunMode(value: unknown): WslRunMode | undefined {
  return value === "sync" || value === "async" || value === "watch" ? value : undefined;
}

function optionalTimeoutBehavior(value: unknown): WslTimeoutBehavior | undefined {
  return value === "kill" || value === "detach" ? value : undefined;
}

function optionalTaskTailChars(args: Record<string, unknown>): number | undefined {
...
async function ensureInstalledDistro(distro: string): Promise<void> {
  const distros = await listDistros();
...
async function resolveRequestedDistro(input?: string): Promise<string | null | undefined> {
...
calls from older examples.`,
  ...
  async (params: { action: string; distro?: string }) => {
    try {
